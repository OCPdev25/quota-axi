...
- `--fail-below <percent>` adds a threshold result to quota output and makes the command usable as a fail-closed shell gate: exit 3 means a measured window is below the floor, while exit 4 means at least one requested provider exposed no measurable percentage. It is intentionally unavailable for `auth` and `watch`.
